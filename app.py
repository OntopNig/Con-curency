    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            try:
                ...
                logger.error(f"[JSON PARSE FAIL] Status={response.status if response else 'N/A'} | Length={len(resp_text) if resp_text else 0} | First500={repr(resp_text[:500]) if resp_text else 'EMPTY'}")
...
def _is_loop_alive():
    """Check if the event loop and its thread are actually running."""
...
def get_event_loop():
    ...
    This is safe across Gunicorn worker forks — if the background 
    thread died during fork, we detect it and recreate everything.
    """
    global _loop, _loop_thread, _semaphore
    with _loop_lock:
        if not _is_loop_alive():
            # Close stale loop if it exists
